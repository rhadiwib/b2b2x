...
public interface ProjectRepository extends ReactiveCrudRepository<Project, Long>, ProjectRepositoryInternal {
    ...
    @Query("SELECT * FROM project entity WHERE entity.id not in (select cpq_quotedetails_id from cpq_quotedetails)")
    Flux<Project> findAllWhereCpqQuotedetailsIsNull();
